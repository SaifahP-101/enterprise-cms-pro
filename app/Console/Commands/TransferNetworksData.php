...
class TransferNetworksData extends Command
{
    /**
     * The name and signature of the console command.
     * (ยกเลิกระบบเคลียร์ข้อมูลเก่าแล้ว)
     *
     * @var string
     */
    protected $signature = 'cms:transfer-networks';
     ...
     * @var string
     */
    protected $description = 'Transfer networks data, extract base64, and copy files to Enterprise CMS (Category 9)';
    ...
    {
        $this->info('🚀 Starting Data, Tags, Base64 & Physical Files Transfer for Networks...');

        // 1. กำหนด Path ต้นทางและปลายทาง
        // ⚠️ หมายเหตุ: ปรับโฟลเดอร์ 'old/images/network' ให้ตรงกับพาธเก็บไฟล์จริงของระบบเก่า
        $oldBasePath        = public_path('old/images/networks');
        $oldBasePathPDF     = public_path('old/images/networks/pdf');
        $oldBasePathGallery = public_path('old/images/networks/gallery');
        ...
        // 2. ดึงข้อมูลจากฐานข้อมูลเก่า DB 1 (ตาราง networks)
        $oldRecords = DB::connection('mysql_old')->table('networks')->get();
        
        if ($oldRecords->isEmpty()) {
            $this->warn('⚠️ No records found in networks table.');
            return 0;
        }
        ...
        foreach ($oldRecords as $old) {
            ...
            if (!empty($old->file_text)) { 
                ...
                if (File::exists($textPath)) {
                    ...
                    // สกัดรูปภาพ Base64 เป็นไฟล์ Physical เพื่อประสิทธิภาพ DB
                    if (!empty($bodyContent) && strpos($bodyContent, 'data:image') !== false) {
                        ...
                        foreach ($images as $img) {
                            ...
                            if (preg_match('/^data:image\/(\w+);base64,/', $src, $type)) {
                                ...
                                // สลับ Src Base64 ให้ชี้ไปที่ URL
            ...
            // 6. จัดการ Slug ให้รองรับภาษาไทย และไม่ซ้ำกัน (ป้องกัน Error 1062 Duplicate Entry)
            ...
            if (!$slug) {
                ...
                $slug = mb_substr($thaiSlug ?: 'network', 0, 100) . '-network-' . $old->id . '-' . Str::random(4);
            }

            // 7. บันทึกข้อมูลลงตาราง Contents 
            $content = Content::create([
                'category_id'      => 9, // 9 = เครือข่ายความร่วมมือ[cite: 2]
                'title'            => strip_tags($old->title), // ป้องกัน XSS
                'slug'             => $slug,
                'type'             => 'NEWS', // อ้างอิงตาม Type ในฐานข้อมูลใหม่[cite: 2]
                'body'             => $bodyContent,
            ...
            // 9. จัดการรูปภาพ Gallery (สมมติระบบเก่าใช้ตาราง network_gallerys)
            if (Schema::connection('mysql_old')->hasTable('network_gallerys')) {
                $newGalleryPath = storage_path('app/public/contents/galleries/' . $content->id);
                
                $oldGalleries = DB::connection('mysql_old')
                    ->table('network_gallerys')
        ...
        $bar->finish();
        $this->newLine();
        $this->info('✅ Networks Data, Tags, Base64 Extraction, and Files Migration Completed!');
        return 0;
    }
}