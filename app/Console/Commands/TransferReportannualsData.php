...
class TransferReportannualsData extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cms:transfer-reportannuals';
     ...
     * @var string
     */
    protected $description = 'Transfer reportannuals data, extract base64, and copy files to Enterprise CMS (Category 10)';
    ...
    {
        $this->info('🚀 Starting Data, Tags, Base64 & Physical Files Transfer for Report Annuals...');

        // 1. กำหนด Path ต้นทางและปลายทาง 
        // ⚠️ หมายเหตุ: ปรับโฟลเดอร์ 'old/images/reportannual' ให้ตรงกับระบบเก่า
        $oldBasePath        = public_path('old/images/reportannuals');
        $oldBasePathPDF     = public_path('old/images/reportannuals/pdf');
        $oldBasePathGallery = public_path('old/images/reportannuals/gallery');
        ...
        // 2. ดึงข้อมูลจากฐานข้อมูลเก่า DB 1 (ตาราง reportannuals)
        $oldRecords = DB::connection('mysql_old')->table('reportannuals')->get();
        
        if ($oldRecords->isEmpty()) {
            $this->warn('⚠️ No records found in reportannuals table.');
            return 0;
        }
        ...
        foreach ($oldRecords as $old) {
            ...
            if (!$slug) {
                ...
                $slug = mb_substr($thaiSlug ?: 'report', 0, 100) . '-report-' . $old->id . '-' . Str::random(4);
            }

            // 7. บันทึกข้อมูลลงตาราง Contents 
            $content = Content::create([
                'category_id'      => 10, // 10 = ประกันคุณภาพการศึกษา
            ...
            if (Schema::connection('mysql_old')->hasTable('reportannual_gallerys')) {
                $newGalleryPath = storage_path('app/public/contents/galleries/' . $content->id);
                
                $oldGalleries = DB::connection('mysql_old')
                    ->table('reportannual_gallerys')
                    ->where('reportannual_id', $old->id) 
                    ->get();
        ...
        $bar->finish();
        $this->newLine();
        $this->info('✅ Report Annuals Data, Tags, Base64 Extraction, and Files Migration Completed!');
        return 0;
    }
}